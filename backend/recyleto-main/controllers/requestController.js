  ...
  try {
    ...
    const newRequest = new Request({
      ...
      medicineDetails: {
        ...
        // Add fallback for when image upload fails
        imageUploadStatus: req.file ? 'success' : 'no_image'
      }
    });
    ...
    let errorMessage = 'Server error while creating medicine request';
    let statusCode = 500;
    
    if (error.message.includes('permission denied') || error.message.includes('EACCES')) {
      errorMessage = 'Permission denied: Unable to save file. The request will be saved without image.';
      statusCode = 400; // Bad request but not server error
    } else if (error.message.includes('ENOENT')) {
      errorMessage = 'Directory not found: Unable to create upload directory.';
    } else if (error.message.includes('No writable upload directory')) {
      errorMessage = 'Server storage error: Unable to find writable directory.';
    }
    ...
    res.status(statusCode).json({
      ...
      suggestion: 'Try submitting the request without an image.'
    });
  }
};
